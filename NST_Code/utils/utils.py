...
def calc_mean_std(feat, eps=1e-5):
    ...
    feat_mean = feat.reshape(batch_size, channels, -1).mean(dim=2).reshape(batch_size, channels, 1, 1)
    feat_var = feat.reshape(batch_size, channels, -1).var(dim=2, unbiased=False) + eps
